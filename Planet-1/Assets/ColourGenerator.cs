...
public class ColourGenerator
{
    ...
    public void UpdateSettings(ColourSettings settings)
    {
        ...
        {
            texture = new Texture2D(TEXTURE_RESOLUTION * 2, settings.biomeColourSettings.biomes.Length, TextureFormat.RGBA32, false);
        }
    ...
    public void UpdateColours()
    {
        ...
        foreach (var biome in settings.biomeColourSettings.biomes)
        {
            for (int i = 0; i < TEXTURE_RESOLUTION * 2; i++)
            {
                ...
                {
                    gradientColour = settings.oceanColour.Evaluate(i / (TEXTURE_RESOLUTION - 1f));
                }
                else
                {
                    gradientColour = biome.gradient.Evaluate((i - TEXTURE_RESOLUTION) / (TEXTURE_RESOLUTION - 1f));
                }
